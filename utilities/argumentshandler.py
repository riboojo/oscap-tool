...
class OscapArguments(object):
    ...
    def start(self):
        ...
        parser = argparse.ArgumentParser(
            description='Simple command line tool for regular openscap scans')

        # Add a subparser for the requested command
        subparsers = parser.add_subparsers(dest='command',
            help='Select one of the following options: scan, history, consult, compare')

        # Handle the scan command subparser
        scan_parser = subparsers.add_parser('scan',
            help='Perform a oscap scan based on provided xccdf file and profile')
        scan_parser.add_argument('-x', '--xccdf', type=str, default='ssg-ol8-xccdf.xml',
            help='Select a valid a XCCDF (located at /usr/share/xml/scap/ssg/content/)')
        scan_parser.add_argument('-p', '--profile', type=str,
            default='xccdf_org.ssgproject.content_profile_stig', help='Select a valid profile')
        ...
        consult_parser = subparsers.add_parser('consult',
            help='Consult a saved scan report by its ID')
        consult_parser.add_argument('-f', '--frm', type=int, required=True,
            help='Select a valid ID (you can execute -history command to retrieve valid IDs)')

        # Handle the compare command subparser
        compare_parser = subparsers.add_parser('compare',
            help='Compare two saved scan reports by their IDs')
        compare_parser.add_argument('-f', '--frm', type=int, required=True,
            help='Select a valid ID (you can execute -history command to retrieve valid IDs)')
        compare_parser.add_argument('-t', '--to', type=int, required=True,
            help='Select a valid ID (you can execute -history command to retrieve valid IDs)')

        # Add an argument for log level configuration
        parser.add_argument('-l', '--loglevel',
            choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'],
            default='INFO', help='Provide a logging level')
        ...
        if not args.command:
            logging.error("Enter a valid command (check --help menu for correct tool handling)")
        else:
